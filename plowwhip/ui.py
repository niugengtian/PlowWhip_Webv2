...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;min-width:320px;min-height:100vh;background:var(--bg)}
    ...
    .project-manager{margin-bottom:12px}.project-manager .panel-body{display:grid;grid-template-columns:minmax(180px,1fr) auto;align-items:end;gap:10px}.project-manager .field{margin:0}
    @media(max-width:1050px){.topbar{grid-template-columns:250px 1fr}.principle{display:none}.metrics-strip,.metrics-five{grid-template-columns:repeat(2,1fr)}.task-workspace{grid-template-columns:1fr 1fr}.task-workspace>.panel:last-child{grid-column:1/-1;max-height:none}.settings-grid{grid-template-columns:1fr}}
    ...
    @media(max-width:520px){main{width:min(100% - 22px,1400px)}.metrics-strip,.metrics-five{grid-template-columns:1fr}.scope-control{width:100%}.scope-control select{min-width:0}.context-actions{width:100%}.facts,.usage-grid,.ratio-grid{grid-template-columns:1fr}.facts>div,.ratio-grid>div{border-right:0!important}.setting-row,.project-manager .panel-body{grid-template-columns:1fr}.top-status{gap:9px;flex-wrap:wrap}.share-row{grid-template-columns:1fr}.share-row strong{text-align:left}}
  </style>
</head>
<body>
  <header class="topbar">
  ...
  <main>
    ...
    <section id="project-butler" hidden>
      <div class="page-heading"><h1 id="butler-title">项目管家</h1><p>只处理当前项目，消息历史保存在 SQLite；一次只接收一条主人指令。</p></div>
      <div class="layout">
    ...
    <section id="project" hidden>
      <div class="page-heading"><h1 id="project-title">项目</h1><p>创建或恢复项目、进入项目、归档无活动 Task 的项目。</p></div>
      <article class="panel project-manager"><header class="panel-heading"><div><span class="kicker">Project Registry</span><h2>项目基本操作</h2></div><span class="muted">归档不删除任何历史</span></header><form class="panel-body" id="create-project-form"><label class="field"><span>项目 ID</span><input id="new-project-id" required pattern="[A-Za-z0-9][A-Za-z0-9._-]{0,63}" placeholder="project-alpha"></label><button class="primary" type="submit">新建 / 恢复</button></form><p class="notice panel-body" id="project-notice" role="status"></p><div class="table"><div class="table-head"><span>项目</span><span>公开状态</span><span>内部阶段</span><span>最近更新</span></div><div id="project-page-list"><div class="empty">正在读取…</div></div></div></article>
    ...
    <section id="token" hidden>
      <div class="page-heading"><h1>Token 计量</h1><p>Cached-input 是 Input 子集；Total = Input + Output。只计量，不参与调度或完成判断。</p></div>
    ...
    <section id="monitor" hidden>
      <div class="page-heading"><h1>Monitor 只读</h1><p>当前权威事实与最近 20 条事件；不保存采样、不回放全历史、不推进状态。</p></div>
  ...
  <script>
    ...
    const show=name=>{views.forEach(view=>view.hidden=view.id!==name);nav.forEach(button=>{const active=button.dataset.view===name;button.classList.toggle('active',active);button.setAttribute('aria-current',active?'page':'false')});if(name==='settings')loadSettings();if(name==='token')loadToken();if(name==='monitor')loadMonitor();if(name==='project-butler')loadButler()};
    ...
    function projectView(data){
      ...
      if(task){const facts=node('dl','facts');facts.append(fact('Task ID',task.id),fact('公开状态',label(task.public_status)),fact('内部阶段',task.phase),fact('Spec Revision',task.spec_revision),fact('Retry',task.retry_count),fact('Outcome',label(task.outcome)));summary.append(facts);const spec=parse(task.spec_json)||{};summary.append(node('p','objective',spec.instruction||spec.content||JSON.stringify(spec)))}
      else summary.append(node('div','empty','项目已创建，尚未收到执行指令。'));
      ...
      if(task){const open=node('button','primary','查看 Task 证据');open.addEventListener('click',()=>openTask(task.id));actions.append(open)}
    ...
    async function loadButler(){
      ...
      $('#butler-title').textContent=`${currentProject} · 项目管家`;
    ...
    async function loadToken(){
      try{const data=await api('/api/token');setTokenMetrics('all',data.all_history);setTokenMetrics('today',data.today);$('#token-today-title').textContent=`今日 · ${data.today.date}`;$('#ratio-all-io').textContent=formatRatio(data.all_history.ratios.input_per_output);$('#ratio-all-cu').textContent=formatRatio(data.all_history.ratios.cached_per_uncached);$('#ratio-today-io').textContent=formatRatio(data.today.ratios.input_per_output);$('#ratio-today-cu').textContent=formatRatio(data.today.ratios.cached_per_uncached);
        ...
        const share=$('#project-share');clear(share);const total=Math.max(1,data.all_history.total_tokens);if(!data.projects.length)share.append(node('div','empty','尚无项目消费。'));data.projects.forEach(item=>{const row=node('div','share-row'),track=node('span','share-track'),fill=node('i');fill.style.width=`${item.total_tokens/total*100}%`;track.append(fill);row.append(node('span','mono',item.project_id),track,node('strong','',`${formatToken(item.total_tokens)} · ${(item.total_tokens/total*100).toFixed(1)}%`));share.append(row)});
        usageRows($('#usage-projects'),data.projects,item=>item.project_id);usageRows($('#usage-tasks'),data.tasks,item=>item.task_id);usageRows($('#usage-models'),data.models,item=>`${item.model} · ${item.provider_key}`);usageRows($('#usage-sessions'),data.sessions,item=>`${item.task_session_id} · Worker ${item.worker_id} (${item.worker_role})`)
      }catch(error){const box=$('#token-trend');clear(box);box.append(node('div','empty',error.message))}
    }
    async function loadMonitor(){
      try{const data=await api('/api/monitor'),summary=data.summary;$('#monitor-project-count').textContent=summary.projects;$('#monitor-archived-count').textContent=`归档 ${summary.archived_projects}`;$('#monitor-task-count').textContent=summary.tasks;$('#monitor-due-count').textContent=summary.due_actions;$('#monitor-lease-count').textContent=summary.active_leases;const ok=data.database.quick_check.length===1&&data.database.quick_check[0]==='ok';$('#monitor-db-status').className=`status-pill ${ok?'status-done':'status-needs_decision'}`;$('#monitor-db-status').textContent=ok?'只读检查通过':'检查异常';
        ...
        const projectBox=$('#monitor-projects');clear(projectBox);const active=data.projects.filter(item=>!item.archived_at);if(!active.length)projectBox.append(node('div','empty','没有活动项目。'));active.forEach(item=>{const card=node('article','artifact-card'),head=node('header');head.append(node('strong','',item.project_id),status(item.public_status));card.append(head,node('small','mono',`${item.phase||'—'} · ${item.task_id||'无 Task'} · ${time(item.updated_at)}`));projectBox.append(card)});
    ...
    $('#project-scope').addEventListener('change',event=>event.target.value?openProject(event.target.value):(currentProject=null,currentTask=null,show('global-butler')));
    ...
    $('#create-project-form').addEventListener('submit',async event=>{event.preventDefault();const id=$('#new-project-id').value,notice=$('#project-notice');try{await api('/api/actions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:'create_project',project_id:id,idempotency_key:key()})});notice.textContent=`${id} 已创建或恢复。`;currentProject=id;await loadProjects();await openProject(id)}catch(error){notice.textContent=error.message}});
