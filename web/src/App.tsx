...
function Board({ tasks, goals, projects, workers, providers, usage, onNavigate, onSelect, onSelectGoal }: { tasks: Task[]; goals: Goal[]; projects: Project[]; workers: Worker[]; providers: Provider[]; usage: Usage | null; onNavigate: (view: View) => void; onSelect: (task: Task) => void; onSelectGoal: (goal: Goal) => void }) {
  ...
  return <>
    ...
    <section className="panel board-panel"><div className="panel-heading"><div><span className="kicker">管家唯一入口</span><h1>提交目标 → 策略路由 → 任务 Gate</h1></div><span className="muted">按体量路由 · 临时 Worker 终态释放</span></div>
...
function WorkersView({ items, providers, busy, onRebind }: { items: { project: Project; worker: Worker }[]; providers: Provider[]; busy: boolean; onRebind: (worker: Worker, provider: string) => void }) {
  ...
  const inspect = async (worker: Worker, cursor = '0:0:0') => {
    ...
    setStream((current) => cursor === '0:0:0' ? nextStream : {
      ...nextStream, items: [...(current?.items ?? []), ...nextStream.items],
    })
  }
  return <section className="panel"><div className="panel-heading"><div><span className="kicker">项目 · 角色 · CLI 会话</span><h1>Worker 状态</h1></div><span className="muted">点击查看 canonical Task / Host 事实</span></div><div className="worker-table"><div className="table-head"><span>项目 / 角色</span><span>Provider</span><span>会话</span><span>状态</span><span>操作</span></div>{items.length ? items.map(({ project, worker }) => <div className="table-row" key={worker.id}><div><strong>{project.name}</strong><small>{roleNames[worker.role] ?? worker.role}</small></div><div><code>{worker.provider}</code><small>第 {worker.session_generation} 代 · rotation {worker.rotation_reason ?? '无'}</small></div><div><code>{worker.external_session_id ?? '等待 CLI 建立'}</code><small>Input {worker.last_input_tokens} · Cached {worker.last_cached_input_tokens} · Uncached {worker.last_uncached_input_tokens} · Output {worker.last_output_tokens}</small><small>Attribution {worker.last_attribution_granularity} / {worker.last_value_classification} · Token 仅计量，不参与调度或终态</small></div><div><StatusDot ok={worker.status === 'idle'} label={worker.status === 'idle' ? '空闲' : worker.status === 'busy' ? '工作中' : '已释放'} />{worker.last_error && <small className="danger-text">{worker.last_error}</small>}</div><div><button type="button" onClick={() => void inspect(worker)}>查看真实进展</button><select disabled={busy || worker.status !== 'idle'} value={worker.provider} onChange={(event) => onRebind(worker, event.target.value)}>{providers.filter((provider) => provider.enabled && provider.transport === 'host-bridge').map((provider) => <option key={provider.name} value={provider.name}>{provider.display_name}</option>)}</select></div></div>) : <div className="empty-state compact"><Robot size={34} /><h2>暂无 Worker</h2><p>任务领取时按需创建，终态后释放。</p></div>}</div>{selected && detail && <section className="work-items"><div className="section-heading"><div><span className="kicker">Canonical Worker detail</span><h2>{selected.id}</h2></div><button type="button" onClick={() => { setSelected(null); setDetail(null); setStream(null) }}>关闭</button></div><div className="facts"><Fact label="Task / revision" value={detail.task ? `${detail.task.title} / ${detail.task.revision}` : '无 active Task'} /><Fact label="TaskSpec / episode" value={`${detail.task?.spec_revision ?? '—'} / ${String(detail.episode?.id ?? '—')}`} mono /><Fact label="Session ownership" value={`${String(detail.ownership.external_session_id ?? detail.ownership.session_id ?? '—')} · generation ${String(detail.ownership.session_generation ?? '—')}`} mono /><Fact label="Provider" value={String(detail.task?.provider ?? detail.worker.provider)} /><Fact label="Host Job identity" value={detail.host_job?.job_id ?? '—'} mono /><Fact label="Dispatch outcome" value={`${detail.host_job?.dispatch_outcome ?? '—'} · deadline ${detail.host_job?.reconciliation_deadline_at ?? '—'}`} /><Fact label="Heartbeat" value={`${detail.host_job?.heartbeat_at ?? detail.worker.last_seen_at ?? '—'}（仅表示存活，不代表完成）`} /><Fact label="真实状态" value={detail.task?.status ?? '无 active Task'} /></div><div className="event-list">{stream?.items.length ? stream.items.map((item, index) => <article key={`${item.ref ?? item.refs?.join(',') ?? item.kind}:${item.offset ?? index}`}><div><strong>{item.kind}</strong><small>{item.ref ?? item.refs?.join(', ') ?? 'canonical stream'}</small></div><pre>{item.text}</pre></article>) : <p className="muted">暂无 canonical event / Host output。</p>}</div>{stream && (stream.has_more || stream.items.length > 0) && <button type="button" onClick={() => void inspect(selected, stream.next_cursor)}>从 cursor 继续</button>}</section>}</section>
}
