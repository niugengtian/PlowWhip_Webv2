...
export function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.taskEvents(selectedTask.id).then(setEvents),
      api.taskArtifacts(selectedTask.id).then(setArtifacts),
    ]).then(() => setArtifactError(null)).catch((reason: unknown) => {
      setArtifacts([])
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {view === 'board' && <Board tasks={visibleTasks} projects={projects} workers={workers.map((item) => item.worker)} providers={providers} usage={usage} onNavigate={setView} onSelect={(task) => { setSelectedTask(task); setView('tasks') }} />}
        {view === 'tasks' && <TasksView tasks={visibleTasks} selected={selectedTask} events={events} artifacts={artifacts} artifactError={artifactError} busy={busy} onSelect={setSelectedTask} onDrive={driveTask} onControl={controlTask} onOpenArtifact={openArtifact} onCopyArtifact={copyArtifactPath} />}
...
function Board({ tasks, projects, workers, providers, usage, onNavigate, onSelect }: { tasks: Task[]; projects: Project[]; workers: Worker[]; providers: Provider[]; usage: Usage | null; onNavigate: (view: View) => void; onSelect: (task: Task) => void }) {
  ...
  return <>
    <section className="metrics-strip">
      ...
      <Metric icon={TerminalWindow} label="可用 Provider" value={providers.filter((p) => p.transport === 'host-bridge' && p.status === 'available').length} hint={`${providers.filter((p) => p.transport === 'host-bridge' && p.enabled).length} 已启用`} onClick={() => onNavigate('providers')} />
...
function ProvidersView({ providers, busy, onProbe, onToggle }: { providers: Provider[]; busy: boolean; onProbe: (provider: Provider) => void; onToggle: (provider: Provider) => void }) {
  const workers = providers.filter((provider) => provider.transport === 'host-bridge')
  return <section className="panel"><div className="panel-heading"><div><span className="kicker">打工仔池</span><h1>Worker Provider</h1></div><span className="muted">所有项目 Worker 均在本机项目目录执行</span></div><div className="provider-grid">{workers.map((provider) => <article className={`provider-card ${provider.enabled ? '' : 'disabled'}`} key={provider.name}><div className="provider-head"><div className="provider-icon"><TerminalWindow size={20} /></div><StatusDot ok={provider.status === 'available'} label={provider.enabled ? provider.status === 'available' ? '可用' : provider.status === 'unknown' ? '待探测' : '不可用' : '已停用'} /></div><h2>{provider.display_name}</h2><code>{provider.name}</code><dl><div><dt>适配器</dt><dd>{provider.adapter}</dd></div><div><dt>运行位置</dt><dd>本机 Host Bridge</dd></div><div><dt>可执行文件</dt><dd>{provider.executable ?? '内置'}</dd></div>{provider.credential_env && <div><dt>凭据入口</dt><dd>{provider.credential_env}</dd></div>}<div><dt>能力</dt><dd>{provider.capabilities.join(' · ')}</dd></div></dl><p className="provider-reason">{provider.reason ?? '已就绪'}{provider.last_probed_at && ` · ${provider.last_probed_at}`}</p><div className="card-actions"><button disabled={busy || !provider.enabled} onClick={() => onProbe(provider)}><Pulse size={16} />0 Token 探测</button><button disabled={busy} onClick={() => onToggle(provider)}>{provider.enabled ? '停用' : '启用'}</button></div></article>)}</div><details className="provider-setup"><summary>Host Bridge 启动命令（macOS / Linux / Windows）</summary><h3>macOS</h3><pre>{`.venv/bin/python -m plow_whip_web.host_bridge \\
