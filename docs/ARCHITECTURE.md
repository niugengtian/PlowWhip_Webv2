...
2. A Project Butler conversation is durable, resumable, and project-scoped. It requires objective, boundaries, and acceptance criteria; the server exposes exactly one active question at a time through the message API. Complete input produces a proposal hash at a deterministic 95% structural-completeness score. A human may continue the same conversation to revise the proposal; only explicit confirmation of the current revision and hash can create a Goal.
