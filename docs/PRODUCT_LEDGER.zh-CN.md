...
当前结论：**22 个功能章节已实现，1 个部分实现，0 个未实现，3 个为对比/治理章节。§20 的中文项目名称与重复项目动作已完成代码和 51 项回归，正在等待 8750 真实数据库/UI 验收；现场证据完成前不恢复 V1 功能完成声明。**
...
| §20 页面和 API | 部分实现 | 七导航、Task 工作台、messages/actions 两类写路由、本地 Session 分段、`unsafe_unknown` 精确 HostJob 恢复；Schema v6 已分离 `display_name`/内部 ID 和 `message`/`action`，语义重复创建返回 `unchanged`；51 项回归通过 | 等待 8750 真实数据库迁移、中文显示、重复提交和管家历史验收 |
...
| H-20260724-16 | 2026-07-24 | 主人现场验证 | 项目名称应允许直接使用“审查代码”等中文名称，不应要求主人理解或输入 ASCII 内部 ID | 代码完成，待现场验收 | Schema v6 `projects.display_name`；创建 UI 只收主人名称并由系统生成内部 ID；选择器/项目/Task/Token/Monitor/管家显示名称；中文创建与 `@审查代码` 路由回归通过 |
| H-20260724-17 | 2026-07-24 | 主人现场验证 | 同一活动项目和同一工作区再次提交时不得重复写创建历史；内部动作不得伪装成主人聊天 | 代码完成，待现场验收 | 同名活动项目且工作区不变返回 `unchanged/message_id=null`，不新增消息；`messages.entry_kind` 区分自然语言和 Action；创建按钮提交期间锁定；回归验证 Action 不进入管家聊天 |
| H-20260724-18 | 2026-07-24 | 主人架构纠正 | “用 SSH 上传本地代码到指定 GitHub 分支”应优先成为 Simple/确定性 Git 发布 Task，而不是交给 Fullstack Provider 写代码 | 待主人决定远端历史 | 8750 Task `6baddc86e4d846438587bf2352c35ade` 已使用 `git_publisher / git_publish`、3 个零模型 Token HostJob；正确 SSH 身份后普通 push 被 non-fast-forward 安全拒绝，远端仍为 `0b06db7`，本地为 `66a1c7f` |
...
| I-20260724-38 | 2026-07-24 | UI 把主人可见项目名称与文件/URL 使用的内部 `project_id` 合并成一个字段，并用 ASCII 正则限制输入 | 中文项目名称被拒绝，迫使主人把“审查代码”改写为 `check-code` | 增加主人可见项目名称；内部稳定 ID 由系统生成或维护；选择器、看板和管家显示名称，路径/API 继续使用安全 ID | 代码已解决，待现场验收 | Schema v6 兼容回填 `display_name=id`；`test_chinese_project_name_and_semantic_create_deduplication` 和 Web API 中文创建通过 |
| I-20260724-39 | 2026-07-24 | 项目创建按钮提交期间不禁用且每次生成新 idempotency key；已存在且绑定未变化时后端仍写 `create_project`，管家又原样显示内部 action content | 一个项目产生多条无意义创建历史，主人误以为重复创建，审计消息污染正常会话 | 同一项目+同一路径按当前状态语义去重；提交期间锁定按钮并复用幂等键；返回 `created/restored/bound/unchanged` 明确结果；管家隐藏确定性 Action | 代码已解决，待现场验收 | 不同幂等键的同语义重复提交仍不新增 message；历史/新 Action 由 `entry_kind=action` 保留审计但不进入 `/api/butler`；51 项回归通过 |
