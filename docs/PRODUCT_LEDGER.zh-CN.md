...
| H-20260724-18 | 2026-07-24 | 主人架构纠正 | “用 SSH 上传本地代码到指定 GitHub 分支”应优先成为 Simple/确定性 Git 发布 Task，而不是交给 Fullstack Provider 写代码 | 待主人决定远端历史 | 8750 Task `6baddc86e4d846438587bf2352c35ade` 使用 `git_publisher / git_publish` 零模型 Token HostJob；正确 SSH 身份后普通 push 被 non-fast-forward 安全拒绝，远端仍为 `0b06db7a14a23836c89e7e583d4512b19c62b30c`；最终提交后再次只读刷新并绑定当前本地 HEAD |
...
| I-20260724-43 | 2026-07-24 | 目标远端 `blue` 已有 SHA `0b06db7`，本地 clean-room HEAD 不包含该提交 | 普通 push 安全拒绝；合并会把目标分支历史迁入 clean-room，实现与冻结边界冲突；强制覆盖会改写远端历史 | 不自动 merge、不 force；保留远端不变并停在 `NeedsDecision`。主人只能精确选择“以当前远端 SHA 为 lease 强制覆盖 blue”或“改发新分支” | 待主人决定 | `git push --dry-run` 与 HostJob `c9f66c46-3a7f-48f8-896e-de573e2bfbf7` 都返回 non-fast-forward；未产生 ModelCall；只读刷新绑定当前本地 HEAD，远端 SHA 未变化 |
