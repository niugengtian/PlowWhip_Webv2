...
| §20 页面和 API | 部分实现 | 七导航、Task 工作台、messages/actions 两类写路由、本地 Session 分段、`unsafe_unknown` 精确 HostJob 恢复；Git NeedsDecision 先显示准确原因、本地/远端 SHA、Evidence 与选项影响，再开放新分支/force-with-lease 精确决定入口 | 中文主人可见项目名称尚未与内部 ID 分离；同项目/路径重复创建动作尚未按语义去重 |
